...
class TimesBlock(nn.Module):
    def __init__(self, configs):  ##configs is the configuration defined for TimesBlock
        ...
        self.k = configs.tn_top_k  ##k denotes how many top frequencies are
        # taken into consideration
        # parameter-efficient design
        self.conv = nn.Sequential(
            Inception_Block_V1(configs.tn_d_model, configs.tn_d_ff,
                               num_kernels=configs.tn_num_kernels),
            nn.GELU(),
            Inception_Block_V1(configs.tn_d_ff, configs.tn_d_model,
                               num_kernels=configs.tn_num_kernels)
        )
...
class TimesNetEmbedder(nn.Module):
    def __init__(self, configs):
        ...
        e_layers = configs.tn_elayers
        enc_in = configs.enc_in
        d_model = configs.tn_d_model
        dropout = configs.dropout

        # stack TimesBlock for e_layers times to form the main part of TimesNet, named model
        self.model = nn.ModuleList([
            TimesBlock(configs) for _ in range(e_layers)
        ])
        ...
        self.enc_embedding = DataEmbedding(
            enc_in, d_model, 'timeF', 'd', dropout
        )
